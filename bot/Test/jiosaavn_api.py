...
class JioSaavnAPI:
    ...
    def extract_song_id_from_url(self, url: str) -> Optional[str]:
        ...
        Args:
            url: JioSaavn song URL
            
        Returns:
            Song ID or None if not found
        """
        try:
            # Common URL patterns
            patterns = [
            ...
            # If no pattern matches, try to extract from the last part of URL
            url_parts = url.split('/')
            if url_parts:
                last_part = url_parts[-1].split('?')[0]
                if last_part and len(last_part) > 5:  # Basic validation
                    logger.info(f"Using last part as song ID: {last_part}")
                    return last_part
